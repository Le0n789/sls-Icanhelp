...
import { ArrowLeft, Calculator, PieChart as PieChartIcon, RotateCcw, ChevronDown, Lock, Unlock, Trash2, Plus, Save, Upload, Download } from 'lucide-react';
...
const INITIAL_PRESETS = [
  { id: '1', model: 'P320', material: 'PA12', density: 0.050, price: 350, ratio: 20 },
  { id: '2', model: 'P360', material: 'PA11', density: 0.060, price: 400, ratio: 30 },
  { id: '3', model: 'P500', material: 'TPU', density: 0.080, price: 500, ratio: 15 },
  { id: '4', model: 'P320', material: 'FS3300PA', density: 0.050, price: 380, ratio: 25 },
];
...
export function CostCalculator() {
  ...
  const [fumigationCount, setFumigationCount] = useState<number | string>('');
  const [fumigationUnitPrice, setFumigationUnitPrice] = useState<number | string>('');
  const [nutCount, setNutCount] = useState<number | string>('');
  const [nutUnitPrice, setNutUnitPrice] = useState<number | string>('');
  const [threadCount, setThreadCount] = useState<number | string>('');
  const [threadUnitPrice, setThreadUnitPrice] = useState<number | string>('');
  const [tappingCount, setTappingCount] = useState<number | string>('');
  const [tappingUnitPrice, setTappingUnitPrice] = useState<number | string>('');
  ...
  const [presets, setPresets] = useState(() => {
    try {
      const saved = localStorage.getItem('cost-calculator-presets');
  ...
  useEffect(() => {
    localStorage.setItem('cost-calculator-presets', JSON.stringify(presets));
  }, [presets]);
  ...
  const results = useMemo(() => {
    ...
    const fCount = Math.max(0, Math.floor(Number(fumigationCount) || 0));
    const fPrice = Math.max(0, Number(fumigationUnitPrice) || 0);
    const nCount = Math.max(0, Math.floor(Number(nutCount) || 0));
    const nPrice = Math.max(0, Number(nutUnitPrice) || 0);
    const thCount = Math.max(0, Math.floor(Number(threadCount) || 0));
    const thPrice = Math.max(0, Number(threadUnitPrice) || 0);
    const taCount = Math.max(0, Math.floor(Number(tappingCount) || 0));
    const taPrice = Math.max(0, Number(tappingUnitPrice) || 0);

    const fumigationSubtotal = fCount * fPrice;
    const nutSubtotal = nCount * nPrice;
    const threadSubtotal = thCount * thPrice;
    const tappingSubtotal = taCount * taPrice;

    // 打印高度(mm) * 单位高度材料密度(kg/mm) * 新粉比例(%) / 100 * 材料价格(元/kg)
    const directMaterialCost = isMaterialCostEnabled ? (h * dMm * (nRatio / 100)) * p : 0;
    const postProcessingCost = isPostProcessingEnabled ? (fumigationSubtotal + nutSubtotal + threadSubtotal + tappingSubtotal) : 0;
  ...
  }, [printHeight, densityPerMm, newPowderRatio, price, quoteMultiplier, taxRate, partWeight, fumigationCount, fumigationUnitPrice, nutCount, nutUnitPrice, threadCount, threadUnitPrice, tappingCount, tappingUnitPrice, isPostProcessingEnabled, isMaterialCostEnabled, isQuickQuoteEnabled]);
                ...
                <div className="overflow-auto max-h-[275px] rounded-xl border border-gray-200 relative">
                  <table className="w-full text-left text-sm text-gray-600 border-collapse whitespace-nowrap">
                    <thead className="bg-gray-50 text-gray-900 sticky top-0 z-10 shadow-[0_1px_0_#e5e7eb]">
                      <tr>
                        {isDeletingPreset && <th className="px-4 py-3 font-medium text-center w-12 text-red-600">删除</th>}
                        <th className="px-4 py-3 font-medium text-center w-20">编辑预设</th>
                        <th className="px-4 py-3 font-medium text-center w-16">序列号</th>
                        <th className="px-4 py-3 font-medium">打印机型</th>
                        <th className="px-4 py-3 font-medium">打印材料</th>
                        <th className="px-4 py-3 font-medium">单位高度材料密度 (kg/mm)</th>
                        <th className="px-4 py-3 font-medium">材料价格 (元/kg)</th>
                        <th className="px-4 py-3 font-medium">新粉比例 (%)</th>
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div>
                  ...
                  {isPostProcessingExpanded && (
                    <div className="grid grid-cols-1 sm:grid-cols-12 gap-6 items-start mt-6">
                      {/* Column 1: Count (takes more space) */}
                      <div className="sm:col-span-5 space-y-6">
                        {renderInput("熏蒸（小件）", fumigationCount, setFumigationCount, "件", "1", 0)}
                        {renderInput("镶嵌螺母", nutCount, setNutCount, "个", "1", 0)}
                        {renderInput("镶嵌牙套", threadCount, setThreadCount, "个", "1", 0)}
                        {renderInput("攻丝", tappingCount, setTappingCount, "个", "1", 0)}
                      </div>
                      
                      {/* Column 2: Unit Price (middle space) */}
                      <div className="sm:col-span-3 space-y-6">
                        {renderInput("单价", fumigationUnitPrice, setFumigationUnitPrice, "元", "0.01", 0)}
                        {renderInput("单价", nutUnitPrice, setNutUnitPrice, "元", "0.01", 0)}
                        {renderInput("单价", threadUnitPrice, setThreadUnitPrice, "元", "0.01", 0)}
                        {renderInput("单价", tappingUnitPrice, setTappingUnitPrice, "元", "0.01", 0)}
                      </div>

                      {/* Column 3: Subtotal (right space) */}
                      <div className="sm:col-span-4 space-y-6">
                        <div className="flex flex-col gap-1.5">
                          <label className="text-sm font-medium text-gray-700">熏蒸成本小计</label>
                          <div className="relative flex items-center">
                            <input
                              type="text"
                              readOnly
                              value={results.fumigationSubtotal.toFixed(2)}
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className="text-sm font-medium text-gray-700">镶嵌螺母成本小计</label>
                          <div className="relative flex items-center">
                            <input
                              type="text"
                              readOnly
                              value={results.nutSubtotal.toFixed(2)}
                              className="w-full rounded-lg border border-transparent bg-gray-50 px-4 py-2.5 text-sm text-gray-900 font-medium focus:outline-none"
                            />
                            <span className="absolute right-4 text-sm text-gray-500 pointer-events-none">
                              元
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className="text-sm font-medium text-gray-700">镶嵌牙套成本小计</label>
                          <div className="relative flex items-center">
                            <input
                              type="text"
                              readOnly
                              value={results.threadSubtotal.toFixed(2)}
                              className="w-full rounded-lg border border-transparent bg-gray-50 px-4 py-2.5 text-sm text-gray-900 font-medium focus:outline-none"
                            />
                            <span className="absolute right-4 text-sm text-gray-500 pointer-events-none">
                              元
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className="text-sm font-medium text-gray-700">攻丝成本小计</label>
                          <div className="relative flex items-center">
                            <input
                              type="text"
                              readOnly
                              value={results.tappingSubtotal.toFixed(2)}
                              className="w-full rounded-lg border border-transparent bg-gray-50 px-4 py-2.5 text-sm text-gray-900 font-medium focus:outline-none"
                            />
                            <span className="absolute right-4 text-sm text-gray-500 pointer-events-none">
                              元
